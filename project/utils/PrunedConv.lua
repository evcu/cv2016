...
local pcv, parent = torch.class('nn.PrunedConv', 'nn.Module')

function pcv:__init(nInputPlane, nOutputPlane, kW, kH, dW, dH, padW, padH)
   parent.__init(self)
...
function pcv:setMask(mask)
   self.mask = mask
   self.weight:cmul(self.mask) 
end

function pcv:reset(stdv)
   print('reset! made l#40')
   if stdv then
...
function pcv:updateOutput(input)
...
function pcv:updateGradInput(input, gradOutput)
   assert(input.THNN, torch.type(input)..'.THNN backend not imported')
   ...
   if self.mask ~= nil then
      print('updateGradInput masked!')
      self.gradWeight:cmul(self.mask)
   end
   return self.gradInput
end

function pcv:accGradParameters(input, gradOutput, scale)
   assert(input.THNN, torch.type(input)..'.THNN backend not imported')
   scale = scale or 1
   backCompatibility(self)
